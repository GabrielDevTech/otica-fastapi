# Endpoints para Frontend - Sprint 1

## 📋 Índice

1. [1º Passo (A): Cadastro de Lojas (stores)](#1º-passo-a-cadastro-de-lojas-stores)
2. [1º Passo (B): Cadastro de Departamentos (departments)](#1º-passo-b-cadastro-de-departamentos-departments)
3. [2º Passo: Finalizar Equipe (staff_members)](#2º-passo-finalizar-equipe-staff_members)
4. [3º Passo: Cadastro de Armações (products_frames)](#3º-passo-cadastro-de-armações-products_frames)
5. [4º Passo: Cadastro de Lentes (products_lenses)](#4º-passo-cadastro-de-lentes-products_lenses)
6. [5º Passo: Cadastro de Clientes (customers)](#5º-passo-cadastro-de-clientes-customers)
7. [Endpoints Existentes (Citados)](#endpoints-existentes-citados)
...
**Telas a Implementar**:
- **Lista de Lojas** (`/lojas`): Tela que exibe todas as lojas da organização em formato de tabela ou cards. Deve mostrar: nome, endereço (formatado do JSON), telefone, taxa da máquina e status (ativa/inativa). Deve ter botão para criar nova loja e ações para editar/excluir cada loja.
...
**Telas a Implementar**:
- **Lista de Membros da Equipe** (`/equipe`): Tela que exibe todos os membros da equipe. Deve mostrar: nome, email, role, cargo (job_title), loja, setor e status. Deve ter filtros de busca (nome/email) e por role. Deve ter botão para criar novo membro e ações para editar cada membro.
...
**Request Body** (todos os campos opcionais, mas `store_id` e `department_id` são obrigatórios se o membro ainda não tiver):
```typescript
{
...
**Descrição**: Obtém um membro específico.
...
## 📋 Endpoints Existentes (Citados)

### Staff (Equipe)

- `GET /api/v1/staff` - Lista membros da equipe
- `GET /api/v1/staff/stats` - Estatísticas da equipe
- `GET /api/v1/staff/me` - Informações do usuário atual

**Páginas Next.js**:
- `/equipe` - Lista de membros
- `/equipe/{id}` - Detalhes do membro

### Stores (Lojas)

- `GET /api/v1/stores/{store_id}` - Obtém loja específica
- `PATCH /api/v1/stores/{store_id}` - Atualiza loja
- `DELETE /api/v1/stores/{store_id}` - Desativa loja
...
- `GET /api/v1/departments/{department_id}` - Obtém setor específico
- `PATCH /api/v1/departments/{department_id}` - Atualiza setor
- `DELETE /api/v1/departments/{department_id}` - Desativa setor

**Páginas Next.js**:
- `/setores/{id}` - Detalhes do setor
- `/setores/{id}/editar` - Editar setor

### Access Requests (Solicitações de Acesso)

- `POST /api/v1/access-requests/public` - Cria solicitação (público)
- `GET /api/v1/access-requests/public/validate-code` - Valida código (público)
- `GET /api/v1/access-requests` - Lista solicitações (admin)
- `POST /api/v1/access-requests/{id}/approve` - Aprova solicitação
- `POST /api/v1/access-requests/{id}/reject` - Rejeita solicitação

**Páginas Next.js**:
- `/solicitar-acesso` - Página pública de solicitação
- `/admin/solicitacoes` - Painel de solicitações (admin)

### Invitations (Convites)

- `POST /api/v1/invitations` - Convita usuário
- `POST /api/v1/invitations/resend/{staff_id}` - Reenvia convite

**Páginas Next.js**:
- `/admin/convites` - Painel de convites (admin)

---

## 🔐 Autenticação

Todos os endpoints (exceto os públicos) requerem:
...
- Obrigatórios ao criar/editar staff
- Devem pertencer à organização do usuário

---

**Documento criado em**: 2025-01-XX  
**Status**: Referência para Frontend  
